...
class Elevator:
    ...
    def move_elevator(self, screen, floor_num, floor):
        ...
        if time_end - self.time_start >= two_seconds:
            ...
            target_y = self.SCREEN_HEIGHT - ((FLOOR_HEIGHT + LINE_HEIGHT) * (floor_num + 1) - LINE_HEIGHT)
            if y_elevator_position != target_y:                                                    # If the elevator has not yet reached its destination
                if target_y > y_elevator_position:
                    new_y = target_y - (FLOOR_HEIGHT + LINE_HEIGHT) * (updates_the_time / 0.5)
                    y_elevator_position = new_y if new_y <= target_y else target_y                 # A computer with the new_y took the place of target_y
                else:
                    new_y = target_y + (FLOOR_HEIGHT + LINE_HEIGHT) * (updates_the_time / 0.5)     # A computer with the new_y took the place of target_y
    ...
    def calculates_the_time(self, floor_num):
        ...
        x_elevator_position, y_elevator_position = self.elevator_position
        elevator_floor =  (self.SCREEN_HEIGHT - y_elevator_position)//(FLOOR_HEIGHT + LINE_HEIGHT)
    ...
    def updates_the_location_of_the_elevator(self, x_elevator_position, y_elevator_position, screen):
        pygame.draw.line(screen, sky_blue, [x_elevator_position + ELEVATOR_WIDTH/2, self.SCREEN_HEIGHT], [x_elevator_position + ELEVATOR_WIDTH/2, 0], 70)
